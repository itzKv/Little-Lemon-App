...
class DishesModel: ObservableObject {
    ...
    func reload(_ coreDataContext:NSManagedObjectContext) async {
        ...
        do {
            let (data, _) = try await urlSession.data(from: url) // Queries from server from url
            
            let decoder = newJSONDecoder()
            let fullMenu = try decoder.decode(JSONMenu.self, from: data)
            menuItems = fullMenu.menu
//            let menuItemsCopy: [MenuItem] = [] // Created blank copy of menuItems 
